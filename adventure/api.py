...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        for p_uuid in nextPlayerUUIDs:
            if player.uuid is not p_uuid:
                pusher.trigger(f'{nextRoomID}', f'message', {'id': f'{uuid.uuid4().hex}','message':f'{player.user.username} just entered {nextRoom.title}.'})
...
def say(request):
    ...
    if message:
        pusher.trigger(f'{player.currentRoom.id}', f'message', {'id': f'{uuid.uuid4().hex}','message':f'{player.user.username} says {message}.'})
